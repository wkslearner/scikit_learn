...
# 装饰器是一种接受函数,当你装饰一个函数，意味着你告诉Python调用的是那个由你的装饰器返回的新函数，
# 而不仅仅是直接返回原函数体的执行结果。
...
'''外部调用预测模型时，需要在服务器终端 运行命令行 python flask_model_api.py'''
if __name__ == '__main__':
    app.config['JSON_AS_ASCII'] = False
    app.run(host='127.0.0.1', port=5003,debug=True)  # 127.0.0.1 #指的是本地ip,在文件内直接运行APP服务
