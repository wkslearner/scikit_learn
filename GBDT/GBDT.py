...
allData = pd.read_excel('application.xlsx',header = 0, encoding = 'latin1')
...
# 将带％的百分比变为浮点数
#trainData['int_rate_clean'] = trainData['int_rate'].map(lambda x: x.replace('%','')/100)
# 将工作年限进行转化，否则影响排序
...
X1 = v.fit_transform(trainData[cat_features].to_dict('records'))  #把类别型变量转换成数值型变量（以哑变量的形式存放）
#将独热编码和数值型变量放在一起进行模型训练
X2 = np.matrix(trainData[num_features])

X = np.hstack([X1,X2])  #矩阵以列形式叠加在一起
y = trainData['y']
# 未经调参进行GBDT模型训练
...
# 考虑earliest_cr_line到申请日期的跨度，以月份记
testData['earliest_cr_to_app'] = testData.apply(lambda x: MonthGap(x['earliest_cr_line'],x['issue_d']), axis = 1)
...
'''
网格搜索后，GBDT模型的最佳参数为：
{'n_estimators': 70} 0.666153019328
{'min_samples_split': 2, 'max_depth': 11} 0.671128613501
{'min_samples_leaf': 10, 'min_samples_split': 80} 0.6720038591
{'max_features': 75} 0.668916544118
{'subsample': 0.85} 0.663758900816
'''

best_n_estimator=70
best_max_depth=11
best_min_samples_leaf=10
best_max_features=75
best_subsample=0.85
...
fpr,tpr,threshold=roc_curve(lr_pred['y'],lr_pred['predprob'])
roc_auc = auc(fpr, tpr)
