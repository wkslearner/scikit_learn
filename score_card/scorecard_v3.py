...
from score_card.scorecard_functions_V3 import *
...
allData = pd.read_excel('application.xlsx')
...
# 将带％的百分比变为浮点数
trainData['int_rate_clean'] = trainData['int_rate'].astype(float)  #.map(lambda x: float(x.replace('%',''))/100)
...
# ### 有些变量不显著，需要逐步剔除
varLargeP = {k: v for k,v in pvals.items() if v >= 0.1}  #选择p值大于0.1的参数
...
model_parameter = {}
for C_penalty in np.arange(0.01, 0.2,0.01):
    for bad_weight in range(2, 101, 2):
        ...
        y_pred = LR_model_2.predict_proba(X_test)[:,1]
...
featureImportance = {features_rfc[i]:RFC.feature_importances_[i] for i in range(len(features_rfc))}
