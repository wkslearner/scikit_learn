...
def offline(X_org, y_org):
    ...
    x_train, y_train, x_valid, y_valid = train_test_split(X_org, y_org, 0.2)
    ...
    for j, clf in enumerate(clfs):
        ...
        for i, (train, test) in enumerate(skf):
            ...
            y_submission = clf.predict_proba(X_test)[:,1]  # k折中数据预测结果
            dataset_blend_train[test, j] = y_submission  # 把预测结果作为训练集
            dataset_blend_test_j[:, i] = clf.predict_proba(X_submission)[:,1]  # 测试集预测结果
        dataset_blend_test[:,j] = dataset_blend_test_j.mean(1) # 每个测试集结果求均值
